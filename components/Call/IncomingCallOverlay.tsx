...
import { Canvas, Circle, Group, Blur, useComputedValue, useClockValue, Skia } from '@shopify/react-native-skia';
...
export const IncomingCallOverlay = () => {
  ...
  const auraRadius = useComputedValue(() => {
    // Pulse animation logic: loops every 800ms (standard heartbeat)
    const t = (clock.current % 800) / 800;
    return 100 + t * 200;
  }, [clock]);

  const auraOpacity = useComputedValue(() => {
    const t = (clock.current % 800) / 800;
    return 1 - t;
  }, [clock]);
